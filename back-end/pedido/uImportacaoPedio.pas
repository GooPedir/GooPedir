...
begin
  Result := Celular;
  // Verifica se o número tem 11 dígitos e se o nono dígito é '9'
  if (Length(Result) = 11) and (Result[3] = '9') then
    Result := Copy(Result, 1, 2) + Copy(Result, 4, 8); // Remove o nono dígito
end;
...
begin
  ...
  try
    ...
    begin
      // Formata a descrição (primeira letra maiúscula)
      ...
      // Adiciona a descrição repetidamente conforme a quantidade
    ...
    // Libera a memória das listas de categorias
    for Categoria in Categorias.Keys do
...
function RemoverAcentos(const Texto: string): string;
const
  ComAcentos = 'áàâãäéèêëíìîïóòôõöúùûüçÁÀÂÃÄÉÈÊËÍÌÎÏÓÒÔÕÖÚÙÛÜÇ%-';
...
begin
  ...
  begin
    ...
    begin
      ...
      // Se não houver extras, define como array vazio
    end;
...
begin
  ...
  Req.BaseURL := 'https://ws.goopedir.com/v2/pedidos.php?codigo=' +
    User.ToString;
  try
    Req.Execute;

    JSONArray := TJSONObject.ParseJSONValue(Req.Retorno) as TJSONArray;

    try
      // Verifica se o JSONArray não é nulo
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            // Acessa o valor do campo "id"
            // Writeln('ID do Pedido: ' + JSONObject.GetValue<string>('id'));
            getPedido(JSONObject.GetValue<string>('id'));
            // TGetPedidoThread.Create(JSONObject.GetValue<string>('id')).Start;
          end;
        end;
      end;
    finally
      // Libera a memória alocada para o JSONArray
      JSONArray.Free;
    end;
  except
