...
begin
  ...
  if not LExtension.isEmpty or Req.RawWebRequest.PathInfo.EndsWith('/') then
    ...
    {$ENDIF}
    LFullPath := StringReplace(LFullPath,'staticjs','static\js\',[rfReplaceAll]);
    LFullPath := StringReplace(LFullPath,'staticcss','static\css\',[rfReplaceAll]);
    LFullPath := StringReplace(LFullPath,'staticmedia','static\media\',[rfReplaceAll]);
