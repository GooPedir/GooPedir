...
{                                                       }
{ Copyright(c) 2004-2015 Embarcadero Technologies, Inc. }
{                                                       }
{*******************************************************}
...
{$REGION 'Dataset lists'}
  ...
  /// <summary> Helper to convert/revert objects for marshalling dataset lists
  /// </summary>
  ...
  /// <summary> Dataset list payload object
  /// </summary>
  /// <remarks> Use TFDJSONDataSetsWriter to write.
  /// </remarks>
  /// <remarks> Use TFDJSONDataSetsReader to read.
  /// </remarks>
  TFDJSONDataSets = class sealed(TFDJSONDataSetsBase)
  end;

  /// <summary> Delta list payload object
  /// </summary>
  /// <remarks> Use TFDJSONDeltasWriter to write.
  /// </remarks>
  /// <remarks> Use TFDJSONDeltasReader to read.
  /// </remarks>
  /// <remarks> Use TFDJSONDeltasApplyUpdates to apply delta.
  /// </remarks>
...
{$REGION 'Reader and Writer interfaces'}
  ...
  /// <summary> Data list payload writer interface
  /// </summary>
  ...
  /// <summary> Delta list payload writer interface
  /// </summary>
  ...
  /// <summary> Dataset payload list reader interface
  /// </summary>
  ...
  /// <summary> Delta list payload reader interface
  /// </summary>
  ...
  /// <summary> Delta list payload applyupdates interface
  /// </summary>
  ...
  /// <summary> Delta list payload applyupdates interface with maxerrors control
  /// </summary>
...
{$REGION 'Reader and Writer base classes'}
  /// <summary> Base class for writers.
  /// </summary>
  ...
  /// <summary> Base class for readers.
  /// </summary>
...
{$REGION 'Reader and Writer classes'}
  /// <summary> Data list payload writer implementation
  /// </summary>
  /// <remarks> Use this class to add to a TFDJSONDataSets. </remarks>
  /// <remarks>Call Create to
  ///  instantiate an IFDJSONDataSetsWriter interface.  Or call class methods.
  /// </remarks>
  ...
  /// <summary> Dataset list payload reader implementation
  /// </summary>
  /// <remarks> Use this class to read a TFDJSONDataSets
  /// </remarks>
  /// <remarks>Call Create to
  ///  instantiate an IFDJSONDataSetsReader interface.  Or call class methods.
  /// </remarks>
  ...
  /// <summary> Delta list payload writer implementation
  /// </summary>
  /// <remarks> Use this class to add to a TFDJSONDeltas
  /// </remarks>
  /// <remarks>Call Create to
  ///  instantiate an IFDJSONDeltasWriter interface.  Or call class methods.
  /// </remarks>
  ...
  /// <summary> Delta list reader implementation
  /// </summary>
  /// <remarks> Use this class to read to a TFDJSONDeltas
  /// </remarks>
  ...
  /// <summary> Delta list payload applyupdates implementation
  /// </summary>
  /// <remarks> Use this class to apply updates from a TFDJSONDeltas
  /// </remarks>
  /// <remarks>Call Create to
  ///  instantiate an IFDJSONDeltasApplyUpdates interface.  Or call class methods.
  /// </remarks>
...
uses
  System.RTLConsts, Data.DBXPlatform,
  System.StrUtils, System.Types, System.Character, FireDAC.DApt.Intf;
...
begin
  ...
  try
    ...
    try
      LMemTable.SaveToStream(LMemoryStream, TFDStorageFormat.sfBinary);
      LMemoryStream.Seek(0, TSeekOrigin.soBeginning);
      SetLength(LBytes, LMemoryStream.Size);
      LMemoryStream.Read(LBytes[0], Length(LBytes));
    finally
      LMemoryStream.Free;
    end;
    //LXMLData := LMemTable.XMLData;
    LBinary64 := EncodeBytes64(LBytes);
  finally
    if LMemTable <> ADataSet then
      LMemTable.Free;
    LAdapter.Free;
  end;
  Result := LBinary64;
...
begin
  LMemTable := TFDMemTable.Create(nil);
  LBytes := DecodeBytes64(AValue);
  LMemoryStream := TMemoryStream.Create;
  try
    if Length(LBytes) > 0 then
    begin
      LMemoryStream.Write(LBytes[0], Length(LBytes));
...
begin
  if ADataList = nil then
    raise EFDDataListError.Create('Cannot write. List is nil');
...
begin
  if (AList = nil) or (AList.FDataSets = nil) then
    raise EFDDataListError.Create('Cannot read. List is nil');
