...
{$ENDIF}
  ...
  private
    ...
    /// </remarks>
    function DataSetToJSONObject(const ADataSet: TDataSet; const AValue: Boolean = True)
      : TJSONObject;
    /// <summary>
    ...
    function DataSetToJSONArray(const ADataSet: TDataSet; const IsChild: Boolean;
      const IsValue: Boolean = True; const IsEncodeBlob: Boolean = True): TJSONArray;
    /// <summary>
...
{$ENDIF}
  ...
  public
    ...
    constructor Create(const ADataSet: TDataSet; const AOnlyUpdatedRecords: Boolean = False;
      ...
      const AEncodeBase64Blob: Boolean = True);
    /// <summary>
    /// Creates an array of JSON objects with all DataSet records.
    /// </summary>
    /// <returns>
...
{$ELSE}
  System.DateUtils, Data.FmtBCD, System.SysUtils, System.StrUtils, System.TypInfo, System.Classes,
...
begin
  ...
  begin
    ...
    if (TDataSetSerializeConfig.GetInstance.&Export.DecimalSeparator <> '') then
      LFormatSettings.DecimalSeparator := TDataSetSerializeConfig.GetInstance.&Export.
        DecimalSeparator;
...
function TDataSetSerialize.DataSetToJSONArray(const ADataSet: TDataSet; const IsChild: Boolean;
  const IsValue: Boolean = True; const IsEncodeBlob: Boolean = True): TJSONArray;
var
  LBookMark: TBookmark;
begin
...
{$ELSE}
      if IsChild and FOnlyUpdatedRecords then
        if (ADataSet.UpdateStatus = TUpdateStatus.usUnmodified) and
      ...
      begin
        case ADataSet.Fields[0].DataType of
          ...
          TFieldType.ftLongWord, TFieldType.ftAutoInc, TFieldType.ftString, TFieldType.ftWideString,
            TFieldType.ftMemo, TFieldType.ftWideMemo, TFieldType.ftGuid:
          ...
          TFieldType.ftSingle, TFieldType.ftFloat:
            begin
              if TDataSetSerializeConfig.GetInstance.Export.FormatFloat.Trim.IsEmpty then
                ...
                Result.Add(FormatFloat(TDataSetSerializeConfig.GetInstance.Export.FormatFloat,
                  ADataSet.Fields[0].AsFloat));
            end;
          TFieldType.ftDateTime, TFieldType.ftTimeStamp:
            begin
              if TDataSetSerializeConfig.GetInstance.DateIsFloatingPoint then
                ...
                Result.Add(DateToISO8601(ADataSet.Fields[0].AsDateTime,
                  TDataSetSerializeConfig.GetInstance.DateInputIsUTC))
              else
                Result.Add(FormatDateTime(TDataSetSerializeConfig.GetInstance.Export.FormatDateTime,
                  ADataSet.Fields[0].AsDateTime));
            end;
          TFieldType.ftTime:
            begin
              if TDataSetSerializeConfig.GetInstance.DateIsFloatingPoint then
                ...
                Result.Add(FormatDateTime(TDataSetSerializeConfig.GetInstance.Export.FormatTime,
                  ADataSet.Fields[0].AsDateTime));
            end;
          TFieldType.ftDate:
            begin
              if TDataSetSerializeConfig.GetInstance.DateIsFloatingPoint then
                ...
                Result.Add(FormatDateTime(TDataSetSerializeConfig.GetInstance.Export.FormatDate,
                  ADataSet.Fields[0].AsDateTime));
            end;
          TFieldType.ftCurrency:
            begin
              if TDataSetSerializeConfig.GetInstance.Export.FormatCurrency.Trim.IsEmpty then
                ...
                Result.Add(FormatCurr(TDataSetSerializeConfig.GetInstance.Export.FormatCurrency,
                  ADataSet.Fields[0].AsCurrency));
            end;
...
function TDataSetSerialize.DataSetToJSONObject(const ADataSet: TDataSet;
  const AValue: Boolean = True): TJSONObject;
var
  ...
  begin
    ...
    begin
      case LField.DataType of
        TFieldType.ftInteger, TFieldType.ftSmallint, TFieldType.ftAutoInc{$IF NOT DEFINED(FPC)},
          TFieldType.ftShortint, TFieldType.ftLongWord, TFieldType.ftWord,
          TFieldType.ftByte{$ENDIF}:
          Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey, TJSONNumber.Create(0));
        TFieldType.ftLargeint:
          Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey, TJSONNumber.Create(0));
      ...
      case LField.DataType of
        TFieldType.ftBoolean:
          Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
            TDataSetSerializeUtils.BooleanToJSON(LField.AsBoolean));
        TFieldType.ftInteger, TFieldType.ftSmallint, TFieldType.ftAutoInc{$IF NOT DEFINED(FPC)},
          TFieldType.ftShortint, TFieldType.ftLongWord, TFieldType.ftWord,
          TFieldType.ftByte{$ENDIF}:
          Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
{$IF DEFINED(FPC)}LField.AsInteger{$ELSE}TJSONNumber.Create(LField.AsInteger){$ENDIF});
        TFieldType.ftLargeint:
          Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
{$IF DEFINED(FPC)}LField.AsLargeInt{$ELSE}TJSONNumber.Create(LField.AsLargeInt){$ENDIF});
{$IF NOT DEFINED(FPC)}TFieldType.ftSingle, TFieldType.ftExtended, {$ENDIF}TFieldType.ftFloat:
          begin
            if TDataSetSerializeConfig.GetInstance.Export.FormatFloat.Trim.IsEmpty then
              Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
{$IF DEFINED(FPC)}TJSONExtFloatNumber.Create(LField.AsFloat){$ELSE}TJSONNumber.Create
                (LField.AsFloat){$ENDIF})
            else
              Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
                TJSONString.Create(FormatFloat(TDataSetSerializeConfig.GetInstance.Export.
                FormatFloat, LField.AsFloat)));
          end;
        TFieldType.ftString, TFieldType.ftWideString, TFieldType.ftMemo, TFieldType.ftWideMemo,
          TFieldType.ftGuid, TFieldType.ftFixedChar, TFieldType.ftFixedWideChar:
          begin
            LStringValue := Trim(LField.AsWideString);
            if (LStringValue.StartsWith('{') and LStringValue.EndsWith('}')) or
              (LStringValue.StartsWith('[') and LStringValue.EndsWith(']')) then
            begin
              try
                Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
{$IF DEFINED(FPC)}GetJSON(LStringValue){$ELSE}TJSONObject.ParseJSONValue
                  (LStringValue){$ENDIF});
              except
                Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
                  TJSONString.Create(LField.AsWideString));
              end;
            end
            else
              Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
        ...
        TFieldType.ftDateTime, TFieldType.ftTimeStamp:
          begin
            if TDataSetSerializeConfig.GetInstance.DateIsFloatingPoint then
              Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
{$IF DEFINED(FPC)}LField.AsFloat{$ELSE}TJSONNumber.Create(LField.AsFloat){$ENDIF})
            else if TDataSetSerializeConfig.GetInstance.DateTimeIsISO8601 then
              Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
                TJSONString.Create(DateToISO8601(LField.AsDateTime,
                TDataSetSerializeConfig.GetInstance.DateInputIsUTC)))
            else
              Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
                TJSONString.Create(FormatDateTime(TDataSetSerializeConfig.GetInstance.Export.
                FormatDateTime, LField.AsDateTime)));
          end;
        TFieldType.ftTime:
          begin
            if TDataSetSerializeConfig.GetInstance.DateIsFloatingPoint then
...
{$IF DEFINED(FPC)}LField.AsFloat{$ELSE}TJSONNumber.Create(LField.AsFloat){$ENDIF})
              ...
              Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
                TJSONString.Create(FormatDateTime(TDataSetSerializeConfig.GetInstance.Export.
                FormatDate, LField.AsDateTime)));
          end;
        TFieldType.ftCurrency:
          begin
            if TDataSetSerializeConfig.GetInstance.Export.FormatCurrency.Trim.IsEmpty then
              Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
{$IF DEFINED(FPC)}TJSONExtFloatNumber.Create(LField.AsCurrency){$ELSE}TJSONNumber.
                Create(LField.AsCurrency){$ENDIF})
            else
              Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
                TJSONString.Create(FormatCurr(TDataSetSerializeConfig.GetInstance.Export.
                FormatCurrency, LField.AsCurrency)));
          end;
        TFieldType.ftFMTBcd, TFieldType.ftBCD:
          Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
{$IF DEFINED(FPC)}BcdToDouble(LField.AsBcd){$ELSE}TJSONNumber.Create
            (BcdToDouble(LField.AsBcd)){$ENDIF});
{$IF NOT DEFINED(FPC)}
...
{$ENDIF}
        TFieldType.ftGraphic, TFieldType.ftBlob, TFieldType.ftOraBlob{$IF NOT DEFINED(FPC)},
          TFieldType.ftStream{$ENDIF}:
          Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LKey,
            TJSONString.Create(IfThen(FEncodeBase64Blob, EncodingBlobField(LField),
            LField.AsString)));
      else
...
{$ENDIF}
          try
            if (not TDataSetSerializeConfig.GetInstance.Export.ExportEmptyDataSet) and
              (LNestedDataSet.RecordCount = 0) then
              Continue;
            if TDataSetSerializeConfig.GetInstance.Export.ExportOnlyFieldsVisible and
              (not HasVisibleFields(LNestedDataSet)) then
              Continue;
            if string(LNestedDataSet.Name).Trim.IsEmpty then
            begin
              Inc(LDataSetNameNotDefinedCount);
              LDataSetName := TDataSetSerializeUtils.FormatDataSetName('dataset_name_not_defined_' +
                LDataSetNameNotDefinedCount.ToString);
            end
            else
              LDataSetName := TDataSetSerializeUtils.FormatDataSetName(LNestedDataSet.Name);
            if TDataSetSerializeConfig.GetInstance.Export.ExportChildDataSetAsJsonObject and
              (LNestedDataSet.RecordCount = 1) then
              Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LDataSetName,
                DataSetToJSONObject(LNestedDataSet))
            else
              Result.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(LDataSetName,
...
{$ENDIF}
  ...
  begin
    ...
    begin
      ...
      LJSONObject.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(FIELD_PROPERTY_ALIGNMENT,
        TJSONString.Create(GetEnumName(TypeInfo(TAlignment), Ord(LField.Alignment))));
      LJSONObject.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(FIELD_PROPERTY_FIELD_NAME,
        TJSONString.Create(LField.FieldName));
      LJSONObject.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(FIELD_PROPERTY_DISPLAY_LABEL,
        TJSONString.Create(LField.DisplayLabel));
      LJSONObject.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(FIELD_PROPERTY_DATA_TYPE,
        TJSONString.Create(GetEnumName(TypeInfo(TFieldType), Integer(LField.DataType))));
      LJSONObject.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(FIELD_PROPERTY_SIZE,
{$IF DEFINED(FPC)}LField.Size{$ELSE}TJSONNumber.Create(LField.Size){$ENDIF});
      LJSONObject.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(FIELD_PROPERTY_ORIGIN,
        TJSONString.Create(LField.ORIGIN));

      if IsPublishedProp(LField, 'Precision') then
        LJSONObject.{$IF DEFINED(FPC)}Add{$ELSE}AddPair{$ENDIF}(FIELD_PROPERTY_PRECISION,
{$IF DEFINED(FPC)}TFloatField(LField).Precision{$ELSE}TJSONNumber.Create(TFloatField(LField)
          .Precision){$ENDIF});
...
{$IF COMPILERVERSION <= 29}
      LJSONObject.AddPair(FIELD_PROPERTY_KEY,
        TJSONString.Create(BoolToStr(pfInKey in LField.ProviderFlags)));
...
{$ENDIF}
{$IF NOT DEFINED(FPC)}
      LJSONObject.AddPair(FIELD_PROPERTY_AUTO_GENERATE_VALUE,
        TJSONString.Create(GetEnumName(TypeInfo(TAutoRefreshFlag),
...
{$ENDIF}
  ...
  constructor TDataSetSerialize.Create(const ADataSet: TDataSet;
    const AOnlyUpdatedRecords: Boolean = False; const AChildRecords: Boolean = True;
    const AValueRecords: Boolean = True; const AEncodeBase64Blob: Boolean = True);
  begin
    FDataSet := ADataSet;
