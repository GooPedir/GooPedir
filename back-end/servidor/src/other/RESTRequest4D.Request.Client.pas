...
uses RESTRequest4D.Request.Contract, Data.DB, REST.Client,
  REST.Response.Adapter, REST.Types, System.SysUtils, System.Classes,
  RESTRequest4D.Response.Contract, System.JSON,
  REST.Authenticator.Basic{$IF CompilerVersion <= 32.0}, IPPeerClient{$ENDIF};
...
type
  ...
  private
    ...
    function RaiseExceptionOn500(const ARaiseException: Boolean)
      : IRequest; overload;
    ...
    function AddParam(const AName, AValue: string;
      const AKind: TRESTRequestParameterKind =
      {$IF COMPILERVERSION < 33}TRESTRequestParameterKind.pkGETorPOST{$ELSE}TRESTRequestParameterKind.pkQUERY{$ENDIF}): IRequest;
    function AddBody(const AContent: string;
      const AContentType: TRESTContentType = ctAPPLICATION_JSON)
      : IRequest; overload;
    ...
    function AddHeader(const AName, AValue: string;
      const AOptions: TRESTRequestParameterOptions = []): IRequest;
    ...
    function Proxy(const AServer, APassword, AUsername: string;
      const APort: Integer): IRequest;
...
uses DataSet.Serialize, System.Generics.Collections, FireDAC.Comp.DataSet,
  FireDAC.Comp.Client, RESTRequest4D.Response.Client,
  RESTRequest4D.Utils;

function TRequestClient.AddBody(const AContent: string;
...
function TRequestClient.AddBody(const AContent: TJSONObject;
...
function TRequestClient.AddBody(const AContent: TJSONArray;
  const AOwns: Boolean): IRequest;
begin
  Result := Self;
  if not Assigned(AContent) then
...
function TRequestClient.AddBody(const AContent: TObject; const AOwns: Boolean)
  : IRequest;
begin
  Result := Self;
  if not Assigned(AContent) then
...
function TRequestClient.AddFile(const AName: string; const AValue: TStream)
  : IRequest;
begin
  Result := Self;
...
function TRequestClient.AddHeader(const AName, AValue: string;
  const AOptions: TRESTRequestParameterOptions): IRequest;
begin
  Result := Self;
...
function TRequestClient.AddParam(const AName, AValue: string;
  const AKind: TRESTRequestParameterKind): IRequest;
begin
  Result := Self;
  if AName.Trim.IsEmpty or AValue.Trim.IsEmpty then
    Exit;
...
function TRequestClient.BasicAuthentication(const AUsername, APassword: string)
  : IRequest;
begin
  Result := Self;
...
begin
  ...
  for I := 0 to Pred(FHeaders.Count) do
    FRESTRequest.Params.Delete(FRESTRequest.Params.ParameterByName
      (FHeaders[I]));
end;
...
begin
  ...
  begin
    if LPart.StartsWith(':') then
      LPreparedUrl := StringReplace(LPreparedUrl, LPart,
        Format('{%s}', [LPart.TrimLeft([':'])]), []);
  end;
  Result := LPreparedUrl;
end;

function TRequestClient.Proxy(const AServer, APassword, AUsername: string;
  const APort: Integer): IRequest;
begin
  Result := Self;
...
const
  REQUEST_DEFAULT_ACCEPT = CONTENTTYPE_APPLICATION_JSON + ', ' +
...
function TRequestClient.RaiseExceptionOn500(const ARaiseException: Boolean)
  : IRequest;
begin
  Result := Self;
...
function TRequestClient.AddBody(const AContent: TStream; const AOwns: Boolean)
  : IRequest;
begin
  Result := Self;
  if not Assigned(AContent) then
...
begin
  ...
  for I := 0 to Pred(ACookies.Count) do
    FRESTRequest.AddParameter(ACookies.Names[I],
      ACookies.Values[ACookies.Names[I]], TRESTRequestParameterKind.pkCOOKIE);
end;
