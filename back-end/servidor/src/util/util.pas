...
procedure MovimentacaoProduto(Pedido, Codigo, Tipo: Integer; Quantidade: Real);
...
begin
  Result := 0;
  CampoDados := Dados.FindField(Campo);
  if (CampoDados = nil) or CampoDados.IsNull then
    Exit;
  Texto := Trim(CampoDados.AsString);
  if Texto = '' then
    Exit;
...
begin
  ...
  // Junta o ID do pedido com o timestamp mantendo o retorno dentro de Integer.
  Valor := ((Timestamp mod 1000000000) * 1000) + Abs(Codigo mod 1000);
  Result := Integer(Valor mod High(Integer));
end;
function GetClientIP(Req: THorseRequest): string;
begin
...
begin
  ...
  begin
    ...
    percentualServico := 0;
    if (Acrecimo > 0) and (Mesa > 0) then
    begin
      try
        percentualServico :=
          strtofloat(conexao.GetParametro('taxa_servico_pedido'));
      except
    ...
    conexao.SQL.Add
      ('valor_taxa_entrega = :taxa, servico_percentual = :taxa_servico, ');
    conexao.SQL.Add
      ('servico = :servico, valor_total_pedido = :total,valor_pedido = :vlpedido, tipo_pagamento = :pag, id_ficha = :ficha, ficha_faturada = :fichafaturada, desc_ficha = :desc, pedido_site = desc_ficha');
    ...
    conexao.Parametros('taxa_servico', percentualServico);
  ...
  begin
    ...
    begin
      ...
      frmServidor.EnviarConferencia(CodigoPedido,
  ...
  try
    ...
    begin
      ...
      begin
        ...
        begin
          conexao.SQL.Add
            ('update pedido set nfce_emite = 3 where codigo = :pedido');
        end
        else
        begin
          conexao.SQL.Add
  ...
  begin
    conexao.SQL.Add
      ('update pedido set pedido_impresso = 1 where codigo = :codigo');
    conexao.Parametros('codigo', CodigoPedido);
    conexao.ExecuteSQL;
  end;
...
begin
  ...
  begin
    ...
    conexao.ExecuteSQL;
    conexao.SQL.Add
      ('update usuario set id_caixa = :id where codigo = :usuario');
    conexao.Parametros('id', Codigo);
    conexao.Parametros('usuario', Usuario);
    conexao.ExecuteSQL;
  end;
...
var
  ...
  DataIni, DataFim, StatusTexto: String;
  function ValorQuery(const Nome: String): String;
  begin
    try
      Result := Trim(Req.Query[Nome]);
    except
      Result := '';
    end;
  end;
...
begin
  ...
  if DataIni = '' then
    DataIni := ValorQuery('dataini');
  if DataIni = '' then
    DataIni := ValorQuery('ini');
  DataFim := ValorQuery('data_fim');
  if DataFim = '' then
    DataFim := ValorQuery('datafim');
  if DataFim = '' then
    DataFim := ValorQuery('fim');
  if (DataIni = '') or (DataFim = '') then
  begin
    Res.Send('Data inicial e final nao informadas').Status(400);
    Exit;
  end;
  Usuario := StrToIntDef(ValorQuery('usuario'), 0);
  StatusTexto := ValorQuery('status');
  Status := StrToIntDef(StatusTexto, -1);
  if (StatusTexto <> '') and (Status < 0) then
  begin
    Res.Send('Status invalido').Status(400);
    Exit;
  end;

  conexao := Tconexao.Create('DoGetResumoCaixa');
  try
    conexao.SQL.Add('SELECT c.id, c.data_abertura, c.hora_abertura,');
    conexao.SQL.Add('c.data_fechamento, c.hora_fechamento,');
    conexao.SQL.Add('c.valor_abertura, c.valor_fechamento,');
    conexao.SQL.Add('u.nome, c.id_site, c.status');
    conexao.SQL.Add('FROM caixa as c');
    conexao.SQL.Add('JOIN usuario as u on u.codigo = c.id_usuario');
    conexao.SQL.Add('WHERE c.data_abertura BETWEEN :data_ini AND :data_fim');
    conexao.Parametros('data_ini', DataIni);
    conexao.Parametros('data_fim', DataFim);
    if Usuario > 0 then
    begin
      conexao.SQL.Add('AND c.id_usuario = :usuario');
      conexao.Parametros('usuario', Usuario);
    end;
    if StatusTexto <> '' then
    begin
...
begin
  ...
  Codigo := MovimentoCaixa(Caixa, 0, 0, 2, Valor,
    'SANGRIA - ' + UpperCase(Descricao), 0);
  frmServidor.ImprimirSangriaGo(Codigo);
...
begin
  ...
  begin
    ...
    Dados.FieldByName('ordem').AsInteger := Sequencial;
    Dados.FieldByName('tipo').AsString := 'Pre�o';
    Dados.FieldByName('extra').AsString := DIASEMANA;
    Dados.FieldByName('descricao').AsString :=
      COPY(DadosConsulta.FieldByName('hora_inicial').AsString, 0, 8) + ' at� ' +
...
begin
  try
    ...
    Res.Send('C�digo N�o Informado').Status(500);
    Exit;
  end;
  ...
  begin
    conexao.SQL.Add
      ('update pedido set valor_total_pedido = (select sum(valor_total) from pedido_produtos where pedido_produtos.codigo_pedido = :cod) where codigo = :cod and id_ficha > 0');
  end;
  ...
  case Tipo of
    1:
      begin
        ...
        begin
          try
            frmServidor.EnviarConferencia(Codigo,
              Req.Headers['user'].ToInteger());
          except
    ...
    3:
      begin
        // Caixa Completo
        frmServidor.ImprimirCaixa(Codigo);
...
begin
  ...
  DescricaoTransferencia := 'Transfer�ncia de ' + DescricaoMesaDe + ' para ' +
    DescricaoMesaPara;
...
begin
  ...
  if frmServidor.urlServicoImpressaoGo <> '' then
  begin
...
begin
  ...
  Numero := StringReplace(Numero, '-', '', [rfReplaceAll]);
  Numero := StringReplace(Numero, ' ', '', [rfReplaceAll]);
  // Verifica se o n�mero possui 11 d�gitos
  if length(Numero) = 11 then
    Result := Format('(%s) %s-%s', [COPY(Numero, 1, 2), COPY(Numero, 3, 5),
      COPY(Numero, 8, 4)])
  else
    Result := Numero; // Retorna o n�mero original se n�o tiver 11 d�gitos
end;
...
var
  ...
  TotalProduto: Real;
  ValorEntrega, ValorAcrescimo, ValorDesconto: Double;
  ClienteNome: String;
  Usuario: Integer;
  imprimir: Boolean;
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        CodigoPedido := GeraCodigoPorDiaPedido(Dados.FieldByName('codigo')
          .AsInteger);
        conexao.SQL.Add
          ('update pedido set tipo_pagamento = :tipo_pagamento, codigo_pedido_dia = :codigo_pedido_dia, status = 1, usuario = :usuario, data_pedido = curdate(), hora_pedido = curtime() where codigo = :codigo');
        conexao.Parametros('usuario', Usuario);
    ...
    begin
      Res.Status(400).Send('{"mensagem":"N�o h� produtos selecionado"}')
    end;
    ...
    conexao.Parametros('desconto', ValorDesconto);
    conexao.Parametros('entrega', ValorEntrega);
    conexao.Parametros('total', (TotalProduto + ValorEntrega + ValorAcrescimo) -
      ValorDesconto);
    conexao.Parametros('acrecimo', ValorAcrescimo);
    conexao.Parametros('codigo', Dados.FieldByName('codigo').AsInteger);
    try
      ...
      DadosTipoPagamento.LoadFromJSON(frmServidor.GetTipopagamento);
      if DadosTipoPagamento.RecordCount > 0 then
      begin
        conexao.Parametros('tipo_pagamento',
          DadosTipoPagamento.FieldByName('codigo').AsInteger);
      end
      else
      begin
        Res.Status(400).Send('{"mensagem":"Tipo de pagamento n�o cadastrado"}')
      end;

      DadosTipoPagamento.Free;
...
procedure DoGetBairrosAtivos(Req: THorseRequest; Res: THorseResponse;
Next: TProc);
var
  conexao: Tconexao;
begin
  conexao := Tconexao.Create('Util');
  try
    conexao.SQL.Add('select * from taxa_entrega');
    conexao.SQL.Add('where ativo = 1');
    conexao.SQL.Add('order by cidade, bairro');
    Res.Send<TJSONArray>(conexao.ConsultaSQL);
  finally
    conexao.Free;
  end;
...
procedure DoGetFichaSabor(Req: THorseRequest; Res: THorseResponse; Next: TProc);
var
  Codigo: Integer;
  conexao: Tconexao;
begin
  ...
  conexao.SQL.Add
    ('select max(id) as id, 0 from sabores_completo where nome = :nome and id_produto = :produto');
  conexao.Parametros('produto', Req.Params['codigo']);
  conexao.Parametros('nome', Req.Params['nome']);
  Res.Send<TJSONArray>(GetFichaSabor(conexao.FieldByName('id')));
  conexao.Free;
end;
...
procedure DoPostFichaSabor(Req: THorseRequest; Res: THorseResponse;
Next: TProc);
var
  Dados, Itens: TFDMemTable;
  conexao: Tconexao;
  JSON: TJSONObject;
  JsonArray: TJSONArray;
  I: Integer;
  idSaborCompleto: Integer;
begin
  // Cria a conex�o
  conexao := Tconexao.Create('Util');
  try
    // L� o JSON da requisi��o
    JSON := TJSONObject.ParseJSONValue(Req.Body) as TJSONObject;
    if not Assigned(JSON) then
      raise Exception.Create('JSON inv�lido.');

    conexao.SQL.Add
      ('select max(id) as id, 0 as zero from sabores_completo where nome = :nome and id_produto = :produto');
    conexao.Parametros('nome', JSON.GetValue<String>('flavor'));
    conexao.Parametros('produto', JSON.GetValue<String>('produto'));
    idSaborCompleto := conexao.FieldByName('id');
    JsonArray := JSON.GetValue<TJSONArray>('itens');

    // Remove os ingredientes existentes para esse sabor
    conexao.SQL.Add
      ('DELETE FROM sabores_completo_ingrediente WHERE id_sabor_completo = :id');
    conexao.Parametros('id', idSaborCompleto);
    conexao.ExecuteSQL;

    // Percorre os itens e insere na tabela
    ...
    begin
      ...
      var
      ID := Item.GetValue<Integer>('id');
      if ID = 0 then
        ID := conexao.GerarID('sabores_completo_ingrediente', 'id');

      conexao.SQL.Add('INSERT INTO sabores_completo_ingrediente ' +
        '(id, id_sabor_completo, id_ingrediente, descricao, ps_grama, vl_total) '
        + 'VALUES (:id, :id_sabor_completo, :id_ingrediente, :descricao, :ps_grama, :vl_total)');

      conexao.Parametros('id', ID);
      conexao.Parametros('id_sabor_completo', idSaborCompleto);
      conexao.Parametros('id_ingrediente',
        Item.GetValue<Integer>('idIngredientes'));
      conexao.Parametros('descricao', Item.GetValue<string>('descricao'));
      conexao.Parametros('ps_grama', Item.GetValue<Double>('quantidade'));
      conexao.Parametros('vl_total', Item.GetValue<Double>('custo'));
      conexao.ExecuteSQL;
    end;
...
begin
  ...
  Sequencial := 0;
  conexao.SQL.Add
    ('select *, saldo_atual as estoque from produto where controle_estoque = 1 and deletado = 0');
  Dados.LoadFromJSON(conexao.ConsultaSQL);
...
var
  ...
  IngredientesAtualizados: TDictionary<Integer, Boolean>;
  Atualizacao: TJSONArray;
  Resultado: TJSONArray;
begin
  Dados := TFDMemTable.Create(nil);
  Resultado := nil;
  IngredientesAtualizados := TDictionary<Integer, Boolean>.Create;
  try
    Dados.LoadFromJSON(Req.Body);
      ...
      begin
        case Dados.FieldByName('TIPO').AsInteger of
          1:
            begin
              // Produto
              MovimentacaoProduto(CodigoRadon(Dados.FieldByName('ID').AsInteger),
                Dados.FieldByName('ID').AsInteger, 1,
                Dados.FieldByName('QTD').AsFloat);
            end
        else
          begin
            CodigoIngrediente := Dados.FieldByName('ID').AsInteger;
            ...
            // Consultar Fator de Conversao antes de fazer a entrada
            MovimentacaoInsulmo(CodigoIngrediente, 1,
              Dados.FieldByName('QTD').AsFloat * Fator,
              Dados.FieldByName('CUSTOTOTAL').AsFloat,
              Dados.FieldByName('CUSTOUN').AsFloat, true);

            // Fazer atualizacao do custo uma vez por ingrediente na requisicao.
            if not IngredientesAtualizados.ContainsKey(CodigoIngrediente) then
            begin
              IngredientesAtualizados.Add(CodigoIngrediente, True);
              Atualizacao := AtualizacaoCustoIngrediente(CodigoIngrediente);
              if Resultado = nil then
...
Next: TProc);
begin
  MovimentacaoProduto((Req.Params['codigo'].ToInteger),
    Req.Params['produto'].ToInteger, 2, Req.Params['qtd'].ToInteger);
end;
...
begin
  ...
  MovimentacaoProduto(CodigoRadon(Produto), Produto, Tipo, Quantidade);
...
var
  hash: String;
begin
  hash := frmServidor.ProdutosHash.Gerar(Req.Params['categoria']);
  frmServidor.ProdutosHash.GerarHash(Req.Params['categoria'], hash);
  Res.Send<TJSONArray>(GetProdutoCategoria(Req.Params['categoria'], hash));
end;

procedure DoGetProdutoBuscaP(Req: THorseRequest; Res: THorseResponse;
Next: TProc);
begin
  Res.Send<TJSONArray>(GetProdutoPesquisa(Req.Params['nome']));
end;
...
begin
  ...
  THorse.Get('/v1/caixa/resumo', DoGetResumoCaixa);
  ...
  // Bairro
  THorse.Get('/v1/util/bairros/ativos', DoGetBairrosAtivos);
  ...
  THorse.Post('v1/util/grava/ingrediente/ficha/sabor', DoPostFichaSabor);
  THorse.Get('/v1/util/grava/ingrediente/ficha/sabor/:codigo/:nome',
    DoGetFichaSabor);
  ...
  THorse.Post('v1/baixa/estoque/produto/:codigo/:produto/:qtd',
...
begin
  ...
  try
    ...
    begin
      ...
      if (DifHoras >= 2) then
      begin
        // Passaram-se mais de 2 horas desde o �ltimo pedido do segundo turno

        frmServidor.CodigoPedido := 0;

      end;

      // Atualiza o �ltimo hor�rio usado para gerar pedidos no turno da tarde
...
begin
  ...
  begin
    MovimentacaoProduto(CodigoRadon(CodigoProduto), CodigoProduto, 1,
      Quantidade);
  end;
...
begin
  ...
  if CodigoProduto > 0 then
  begin
    MovimentacaoProduto(CodigoRadon(CodigoProduto), CodigoProduto, 2,
      Quantidade);
  end;
  conexao.Free;
end;
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        conexao.SQL.Add
          ('update produto set valor_custo = :custo where codigo = :codigo');
...
begin
  ...
  try
    ...
    XML.Add('<produtos_estoque_baixo gerado_em="' +
      FormatDateTime('yyyy-mm-dd"T"hh:nn:ss', Now) + '" minimo_padrao="' +
      FormatFloat('0.######', EstoqueMinimoPadrao, FS) + '">');
    Dados.First;
    while not Dados.Eof do
    begin
      XML.Add('  <produto>');
      XML.Add('    <codigo>' + Dados.FieldByName('codigo').AsString +
        '</codigo>');
      XML.Add('    <nome>' + EscapeXML(Dados.FieldByName('nome_produto')
        .AsString) + '</nome>');
      XML.Add('    <saldo_atual>' + FormatFloat('0.######',
        Dados.FieldByName('saldo_atual').AsFloat, FS) + '</saldo_atual>');
      XML.Add('    <estoque_minimo>' + FormatFloat('0.######',
        Dados.FieldByName('estoque_minimo_usado').AsFloat, FS) +
        '</estoque_minimo>');
      XML.Add('  </produto>');
      Dados.Next;
    end;
    XML.Add('</produtos_estoque_baixo>');
    Result := XML.Text;
  finally
    XML.Free;
  end;
...
begin
  ...
  TThread.CreateAnonymousThread(
    ...
    begin
      ...
      MovimentacaoProduto(CodigoRadon(Produto), Produto, 1, Quantidade);
      conexao.SQL.Add('INSERT INTO alerta_sistema');
      conexao.SQL.Add('(tipo, origem, referencia_id, payload)');
      ...
      conexao.Parametros('mensagem',
        'Produto retornado ao estoque por pedido nao concluido/cancelado.');
      conexao.Parametros('pedido_produto', Pedido);
      conexao.Parametros('produto', Produto);
      conexao.Parametros('quantidade', Quantidade);
      conexao.ExecuteSQL;
      Dados := TFDMemTable.Create(nil);
...
begin
  ...
  // Inicialize a express�o regular e busque a correspond�ncia na URL
  RegEx := TRegEx.Create(Pattern);
  Match := RegEx.Match(URL);

  // Se houver uma correspond�ncia, retorne o n�mero encontrado
  if Match.Success then
    Result := Match.Groups[1].Value
  else
    Result := '';
  // Ou voc� pode lan�ar uma exce��o ou definir uma mensagem de erro
end;
...
// 'Domingo,Segunda,Ter�a,Quarta,Quinta,Sexta,Sabado', '0', '0',
...
begin
  try
    ...
    begin
      ...
      // O segundo par�metro indica que deve excluir todos os arquivos e subpastas
    end;
  except
...
var
  ...
  // Lista corrigida: cada item do loop ser� gravado aqui
...
begin
  ...
  try
    ...
    ListaItens := TList<TItemProcesso>.Create;
    ...
    begin
      ...
      begin
        ...
        try
          Pizza := JsonObj.GetValue<string>('pizza');
        except
          Pizza := '';
        end;
        ...
        // ------------------------------------------
        // 1. CALCULANDO VALOR DO PRODUTO / PIZZA
        // ------------------------------------------
        if Pizza <> '' then
        begin
        ...
        begin
          ...
          if Pedido = 0 then
            Pedido := NovoPedido(conexao.GerarID('pedido', 'codigo'),
              Mesa, QRY);
        end;

        // ------------------------------------------
        // 3. GERAR UUID SE VAZIO
        // ------------------------------------------
        if UUID = '' then
        begin
          CreateGUID(ID);
        ...
        // ------------------------------------------
        // 4. VERIFICAR SE PEDIDO-PRODUTO J� EXISTE
        // ------------------------------------------
        ...
        begin
          // ------------------------------------------
          // 5. INSER��O DO PRODUTO
          // ------------------------------------------
          conexao.SQL.Clear;
          conexao.SQL.Add
            ('insert into pedido_produtos (codigo,codigo_pedido,codigo_produto,valor_unitario,quantidade,'
            + 'valor_total,valor_adicional,impresso,html,usuario,uuid) ' +
          ...
          conexao.SQL.Add
            ('update impressao_pedido_produto set status = 1 where id_pedido = :pedido');
          ...
          // ================================================
          // AGORA GRAVA O ITEM NA LISTA PARA PROCESSAR DEPOIS
          // ================================================
          Item.CodigoPedidoProduto := CodigoPedidoProduto;
    ...
    // =====================================================
    // PROCESSAMENTO FINAL � THREAD COM LISTA DE ITENS
    // =====================================================
    TThread.CreateAnonymousThread(
      procedure
      var
        ...
        CodigoAux: Integer;
      begin
        conexaoT := Tconexao.Create('Util');
        try
          for it in ListaItens do
          begin
            // ---------- ADICIONAIS ----------
            if it.Adicionais <> '' then
              Adiciona(it.Adicionais, conexaoT.CriaQRY, conexaoT.CriaQRY,
                it.CodigoPedidoProduto, conexaoT);
    ...
    QRY.Free;
    // LOG DA OPERA��O (ANTES DE PROCESSAR)
//    conexao := Tconexao.Create('Util');
//    conexao.SQL.Clear;
//    conexao.SQL.Add
...
var
  ...
  ToUpper: Boolean;
begin
  Result := LowerCase(Input); // Primeiro, colocamos tudo em min�sculas
  ToUpper := true;
  for I := 1 to length(Result) do
  begin
    if ToUpper and (Result[I] <> ' ') then
    begin
      Result[I] := UpCase(Result[I]); // Converte para mai�scula
      ToUpper := false;
    end
    else if Result[I] = ' ' then
    begin
      ToUpper := true; // Ativa para mai�scula a pr�xima letra ap�s o espa�o
    end;
  end;
end;
...
var
  conexao: Tconexao;
begin
  conexao := Tconexao.Create('atualizaDadosMesa');
...
begin
  QRY.SQL.Text := 'select concat(mt.descricao, ' + QuotedStr(' ') +
    ', m.nr_mesa,' + QuotedStr(' ') + ',m.descricao) as descricao, 0 as zero ' +
  ...
  QRY.Open;
  DescricaoMesa := Trim(UpperCase(QRY.FieldByName('descricao').AsString));
  QRY.Close;

  if Mesa > 0 then
  begin
    QRY.SQL.Text :=
      'select codigo from pedido where id_ficha = :mesa and codigo_pedido_dia = 0 and status = -1 and origem = 3 and id_caixa is null and data_pedido >= "2026-03-12"';
    ...
    begin
      ...
      begin
        ...
        begin
          ...
          QRY.SQL.Text := '';
          QRY.SQL.Add
            ('select 0 as zero, concat(mt.descricao," ", m.nr_mesa) as descricao from mesa as m');
          QRY.SQL.Add
            ('join mesa_tipo as mt on mt.id_mesa_tipo = m.fk_tipo_mesa');
          QRY.SQL.Add('where m.id_mesa = :nr');
          QRY.ParamByName('nr').AsInteger := Mesa;
          QRY.Open;
          if QRY.RecordCount > 0 then
          begin
    ...
    begin
      ...
      QRY.SQL.Add
        ('select 0 as zero, concat(mt.descricao," ", m.nr_mesa) as descricao from mesa as m');
      QRY.SQL.Add('join mesa_tipo as mt on mt.id_mesa_tipo = m.fk_tipo_mesa');
      QRY.SQL.Add('where m.id_mesa = :nr');
      QRY.ParamByName('nr').AsInteger := Mesa;
      QRY.Open;
      if QRY.RecordCount > 0 then
