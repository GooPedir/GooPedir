...
type
  ...
  private
    ...
    procedure ConsultarAtualizacaoPDV;
    procedure ReportarErroCentralAsync(const AOrigem: string;
  ...
  var
    ...
    AtualizacaoConsultada: Boolean;
    AtualizacaoDisponivel: Boolean;
    AtualizacaoObrigatoria: Boolean;
    AtualizacaoCodigo: Cardinal;
    AtualizacaoVersao: String;
    AtualizacaoReleaseId: String;
    AtualizacaoNotas: String;
    AtualizacaoMensagem: String;
    AtualizacaoMensagemErro: String;
    AtualizacaoDataConsulta: TDateTime;
    AtualizacaoEmAndamento: Boolean;
...
begin
  ...
  begin
    ...
    begin
      ...
      if P = 0 then
        P := Pos(':', Parametro);
      if (P > 0) and (SameText(Copy(Parametro, 1, P - 1), 'port') or
...
begin
  ShellExecute(handle, 'open', PChar(Nome), PChar(Parametros), nil,
    SW_SHOWNORMAL);
end;
...
var
  Ex: Exception;
  IdentificacaoErro, MensagemErro: string;
begin
  IdentificacaoErro := Identificacao;
  MensagemErro := Erro;
  Ex := Exception.Create(Erro);
  try
    ReportarErroCentralAsync('AddErro: ' + Identificacao, Ex);
  finally
    Ex.Free;
  end;
    ...
    begin
      try
        EnviaGlitchtip
          ...
          'Erro', IdentificacaoErro, MensagemErro);
      except
      end;
    end).Start;
end;
...
procedure TfrmServidor.ApplicationEvents1ActionExecute(Action: TBasicAction;
var Handled: Boolean);
begin
  // Erros
end;

procedure TfrmServidor.ApplicationEvents1Exception(Sender: TObject;
E: Exception);
begin
  ReportarErroCentralAsync('ApplicationEvents.OnException', E);
end;

function TfrmServidor.AtualizacaoStatus: TJsonObject;
begin
  Result := TJsonObject.Create;
  Result.AddPair('consultada', TJSONBool.Create(AtualizacaoConsultada));
  Result.AddPair('emAndamento', TJSONBool.Create(AtualizacaoEmAndamento));
  Result.AddPair('atualizado', TJSONBool.Create(AtualizacaoConsultada and
    not AtualizacaoDisponivel and not AtualizacaoEmAndamento and
    (AtualizacaoMensagemErro = '')));
  Result.AddPair('disponivel', TJSONBool.Create(AtualizacaoDisponivel));
  Result.AddPair('obrigatoria', TJSONBool.Create(AtualizacaoObrigatoria));
  Result.AddPair('codigo', TJSONNumber.Create(AtualizacaoCodigo));
  Result.AddPair('versao', AtualizacaoVersao);
  Result.AddPair('releaseId', AtualizacaoReleaseId);
  Result.AddPair('notas', AtualizacaoNotas);
  Result.AddPair('mensagem', AtualizacaoMensagem);
  Result.AddPair('mensagemErro', AtualizacaoMensagemErro);
  if AtualizacaoDataConsulta > 0 then
    Result.AddPair('dataConsulta', FormatDateTime('yyyy-mm-dd hh:nn:ss',
      AtualizacaoDataConsulta))
  else
    Result.AddPair('dataConsulta', '');
end;
...
var
  ...
  begin
    ...
    AddLog(Format
      ...
      [TraceId, Codigo, Usuario, CurrentStep, StringReplace(E.Message, '"',
      '\"', [rfReplaceAll])]));
  end;
...
begin
  ...
  try
    try
      ...
      Objeto.AddPair('mesa', Trim(QryPedido.FieldByName('desc_ficha')
        .AsString));
      Objeto.AddPair('taxa_servico_percent',
        QryPedido.FieldByName('percentual').AsFloat);
      Objeto.AddPair('taxa_servico_valor',
        QryPedido.FieldByName('servico').AsFloat);
      Objeto.AddPair('total_produtos', QryPedido.FieldByName('produto')
        .AsFloat);
      ...
      QryImpressora.SQL.Add
        ('        OR (:codigo = 0 AND impressora_padrao = 1)');
      QryImpressora.SQL.Add('      )');
      ...
      QryImpressora.Open;
      EndStep(Format('rows=%d driver="%s" tipo=%d', [QryImpressora.RecordCount,
        SafeFieldString(QryImpressora, 'driver'),
      ...
      Objeto.AddPair('modelo',
        ModeloImpressora(QryImpressora.FieldByName('tipo_impressao')
        .AsInteger));
      ...
      begin
        ...
        ObjetoCliente.AddPair('celular', QryPedido.FieldByName('celular')
          .AsString);
        ObjetoCliente.AddPair('pedidos', QryPedido.FieldByName('pedidos')
          .AsString);
        ObjetoCliente.AddPair('fidelidade', QryPedido.FieldByName('fidelidade')
          .AsString);
        Objeto.AddPair('sequencial', QryPedido.FieldByName('sequencial')
          .AsInteger);
        ...
        try
          Objeto.AddPair('taxa_entrega',
            QryPedido.FieldByName('entrega').AsFloat);
        except
          Objeto.AddPair('taxa_entrega', 0);
        end;
        Objeto.AddPair('nfceChave', QryPedido.FieldByName('nfce_chave')
          .AsString);
        Objeto.AddPair('nfceProtocolo', QryPedido.FieldByName('nfce_protocolo')
          .AsString);
        Objeto.AddPair('nfceNumero', QryPedido.FieldByName('nfce_numero')
          .AsString);
        EndStep;
        ...
        begin
          ...
          ObjetoEndereco.AddPair('numero', QryPedido.FieldByName('numero')
            .AsString);
          ObjetoEndereco.AddPair('complemento',
            QryPedido.FieldByName('complemento').AsString);
      ...
      begin
        ...
        begin
          ...
          ObjetoPagamento.AddPair('descricao',
      ...
      begin
        ...
        try
          ObjetoPagamento := TJsonObject.Create;
          ObjetoPagamento.AddPair('descricao',
            QryPedido.FieldByName('descricao').AsString);
          ObjetoPagamento.AddPair('transacao',
            QryPedido.FieldByName('transacao').AsString);
          ObjetoPagamento.AddPair('troco',
            QryPedido.FieldByName('troco').AsFloat);
...
begin
  ...
  ConfigurarParametrosAtualizacaoPadrao;
  ConsultarAtualizacaoPDV;
  ...
  // Middlewares
  InicializarLogOperacao;
  THorse.Use(AuthTokenUserMiddleware);
...
var
  ReleaseId: string;
begin
  if AtualizacaoEmAndamento then
    Exit;

  ReleaseId := AtualizacaoReleaseId;
  if ReleaseId.Trim = '' then
  begin
    AtualizacaoMensagemErro :=
      'releaseId não informado para iniciar atualização';
    Exit;
  end;

  AtualizacaoEmAndamento := True;
  AtualizacaoMensagem := 'Atualização em andamento';
  AtualizacaoMensagemErro := '';
  ...
  TThread.CreateAnonymousThread(
    ...
    var
      Con: Tconexao;
      Config: TAtualizacaoConfig;
      AtualizacaoPDV: TAtualizacaoPDV;
      MensagemErro: string;
      Sucesso: Boolean;
    begin
      Con := nil;
      ...
      try
        try
          Con := Tconexao.Create('IniciarAtualizacaoPDV');
          Config := TAtualizacaoConfig.Create('', '', GetBuildInfo);
          Config.ConfigurarBanco(Con.Servidor, StrToIntDef(Con.Porta, 3306),
            Con.Usuario, Con.Senha, Con.NomeBanco);
          AtualizacaoPDV := TAtualizacaoPDV.Create(Config, self.ATUALIZADOR);
          Config := nil;
          Sucesso := AtualizacaoPDV.IniciarAtualizacao(ReleaseId, MensagemErro);
          if Sucesso then
            MensagemErro := ''
          else if MensagemErro = '' then
            MensagemErro := 'Falha ao iniciar Atualizador.exe';
        except
          on E: Exception do
      ...
      TThread.Synchronize(TThread.CurrentThread,
        ...
        begin
          ...
          begin
            AtualizacaoEmAndamento := True;
            AtualizacaoConsultada := True;
            AtualizacaoMensagem := 'Atualizador iniciado. Aguardando reinício do servidor';
            AtualizacaoMensagemErro := '';
            AddLog('Atualizador iniciado. Encerrando servidor para aplicar atualização.');
            TThread.CreateAnonymousThread(
              procedure
              begin
                Sleep(1000);
          ...
          begin
            ...
            AtualizacaoMensagem := 'Falha ao iniciar atualização';
            AtualizacaoMensagemErro := MensagemErro;
            AddLog('Falha ao iniciar atualização do PDV: ' + MensagemErro);
          end;
          AtualizacaoDataConsulta := Now;
        end);
    end).Start;
end;

procedure TfrmServidor.ConsultarAtualizacaoPDV;
begin
  AtualizacaoConsultada := false;
  AtualizacaoDisponivel := false;
  AtualizacaoObrigatoria := false;
  AtualizacaoCodigo := 0;
  AtualizacaoVersao := '';
  AtualizacaoReleaseId := '';
  AtualizacaoNotas := '';
  AtualizacaoMensagem := 'Consultando atualização';
  AtualizacaoMensagemErro := '';
  AtualizacaoDataConsulta := 0;
  ...
  TThread.CreateAnonymousThread(
    ...
    var
      Config: TAtualizacaoConfig;
      AtualizacaoPDV: TAtualizacaoPDV;
      Resultado: TConsultaAtualizacao;
      Mensagem: string;
      MensagemErro: string;
    begin
      Config := nil;
      ...
      try
        ...
        Resultado := AtualizacaoPDV.Consultar;
        if Resultado.MensagemErro <> '' then
          Mensagem := Resultado.MensagemErro
        else if Resultado.Disponivel then
          Mensagem := 'Atualização disponível: ' + Resultado.Versao
        else
          Mensagem := 'Sem atualização disponível';

        TThread.Synchronize(TThread.CurrentThread,
          procedure
          begin
            AtualizacaoConsultada := True;
            AtualizacaoDisponivel := Resultado.Disponivel;
            AtualizacaoObrigatoria := Resultado.Obrigatoria;
            AtualizacaoCodigo := Resultado.Codigo;
            AtualizacaoVersao := Resultado.Versao;
            AtualizacaoReleaseId := Resultado.ReleaseId;
            AtualizacaoNotas := Resultado.Notas;
            AtualizacaoMensagem := Mensagem;
            AtualizacaoMensagemErro := Resultado.MensagemErro;
            AtualizacaoDataConsulta := Now;

            if not mAtualizacao.Active then
              mAtualizacao.Open;
            mAtualizacao.Append;
            mAtualizacaoid.AsInteger := mAtualizacao.RecordCount + 1;
            mAtualizacaoarquivo.AsString := 'Atualizador.exe';
            mAtualizacaopercentual.AsInteger := 0;
            mAtualizacaostatus.AsString := Mensagem;
            mAtualizacaotamanho.AsString := '';
            mAtualizacaouuid.AsString := Resultado.ReleaseId;
            mAtualizacaodata.AsDateTime := Now;
            mAtualizacao.Post;
          end);
      except
        on E: Exception do
        begin
          ...
          TThread.Synchronize(TThread.CurrentThread,
            ...
            begin
              ...
              AtualizacaoDisponivel := false;
              AtualizacaoObrigatoria := false;
              AtualizacaoCodigo := ATU_ERRO_CRITICO;
              AtualizacaoMensagem := 'Falha ao consultar atualização do PDV';
              AtualizacaoMensagemErro := MensagemErro;
              AtualizacaoDataConsulta := Now;
              AddLog('Falha ao consultar atualização do PDV: ' + MensagemErro);
            end);
        end;
      end;
...
procedure TfrmServidor.ReportarErroCentralAsync(const AOrigem: string;
AException: Exception);
var
  Origem, ClasseErro, MensagemErro: string;
  Janela: NativeUInt;
begin
  ...
  Origem := AOrigem;
  ClasseErro := AException.ClassName;
  MensagemErro := AException.Message;
  Janela := handle;
  ...
  TThread.CreateAnonymousThread(
    ...
    begin
      ...
      try
        try
          Config := TAtualizacaoConfig.Create('', '', GetBuildInfo);
          StackTrace := Origem + sLineBreak + ClasseErro + ': ' + MensagemErro;
          Erro := Exception.Create(MensagemErro);
          TAtualizacaoPDV.ReportarErro(Config, StackTrace, '', '', '', '', 0,
            Erro, false, Janela);
        except
          on E: Exception do
            AddLog('Falha ao enfileirar erro na central: ' + E.Message);
        end;
      finally
        Erro.Free;
        Config.Free;
...
procedure TfrmServidor.AuthTokenUserMiddleware(Req: THorseRequest;
  Res: THorseResponse; Next: TProc);
var
  ...
  HeaderUsuario: Integer;
  Jwt: TJWT;
begin
  Authorization := Trim(Req.Headers['Authorization']);
  HeaderUser := Trim(Req.Headers['user']);
  if HeaderUser = '' then
    HeaderUser := Trim(Req.Headers['usuario']);
  ...
  if not StartsText('Bearer ', Authorization) then
  begin
    Res.Send('Nao autenticado').Status(THTTPStatus.Unauthorized);
    raise EHorseCallbackInterrupted.Create;
  end;

  CompactToken := Trim(Copy(Authorization, Length('Bearer ') + 1,
    MaxInt));
  Jwt := nil;
  try
    try
      Jwt := TJOSE.Verify(CHAVE_SECRETA, CompactToken);

      if Jwt.Claims.HasExpiration and (Jwt.Claims.Expiration <= now) then
      begin
        Res.Send('Nao autenticado').Status(THTTPStatus.Unauthorized);
        raise EHorseCallbackInterrupted.Create;
      end;
      ...
      if HeaderUser <> '' then
      begin
        HeaderUsuario := StrToIntDef(HeaderUser, 0);
        TokenUsuario := StrToIntDef(Jwt.Claims.Subject, 0);
        try
          if Jwt.Claims.JSON.GetValue('usuario') <> nil then
            TokenUsuario := Jwt.Claims.JSON.GetValue<Integer>('usuario');
        except
        end;

        if (HeaderUsuario <= 0) or (TokenUsuario <= 0) or
          (HeaderUsuario <> TokenUsuario) then
        begin
          Res.Send('Nao autenticado').Status(THTTPStatus.Unauthorized);
          raise EHorseCallbackInterrupted.Create;
        end;
