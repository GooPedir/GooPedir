...
type
  ...
  TfrmACBrNFe = class(TForm)
    ...
    tEmissao: TTimer;
    iEmissao: iRequisicao;
    dadosEmissao: TFDMemTable;
    TrayIcon1: TTrayIcon;
    tMinimiza: TTimer;
    cCont: TCheckBox;
    dadosCode: TFDMemTable;
    iCode: iRequisicao;
    iContabilidade: iRequisicao;
    notasContabilidade: TFDMemTable;
    ...
    procedure tEmissaoV2Timer(Sender: TObject);
  private
  ...
  public
    ...
    procedure ProcessaNFCe(EMAIL, CNPJ, NOME, Data: String);
    procedure GerarContabil;
    function Internet: Boolean;
    function ExtrairChaveNFe(const ATexto: string): string;
    procedure EnviarNotaFiscal(CNPJ, Data, Hora, Chave, Caminho: String;
      Valor: Real);
    ...
    // Versão Melhorada
    procedure GetDadosReq(Mem: TFDMemTable; URL: String);
    procedure PostDadosReq(URL, BODY: String);
  end;
...
var
  ...
  VlrProdutos, VlrTotalDesconto, VlrTotalOutros, VlrTotalNF, VlrBaseICMS,
    VlrICMS, VlrPis, VlrCofins: Double;
...
begin
  ...
  Requisicao.URL := 'nfce/pedido/produtos/' + NumDFe;
  Requisicao.MemTable2 := Memory;
  Requisicao.Metodo := mGet;
  Requisicao.Execute;

  Requisicao.URL := 'nfce/pedido/outras/' + NumDFe;
  Requisicao.MemTable2 := MemoryOutros;
  Requisicao.Metodo := mGet;
  Requisicao.Execute;
  TaxaDesconto := 0;
  VlrProdutos := 0;
  ...
  begin
    ...
    begin
      ...
      VlrProdutos := VlrProdutos + (Memory.FieldByName('value').AsFloat *
        Memory.FieldByName('quanty').AsFloat);
      Memory.Next;
    end;
  end
  else
  ...
  TotalPago := 0;
  while not MemoryPagamento.Eof do
  begin
    TotalPago := TotalPago + RoundTo(MemoryPagamento.FieldByName('valor')
      .AsFloat, -2);
    MemoryPagamento.Next;
  end;

  MemoryPagamento.First;
  Memory.First;

  if MemoryOutros.RecordCount > 0 then
  begin
    ACBrPosPrinter1.Desativar;
    ACBrPosPrinter1.Porta := 'RAW:' + MemoryOutros.FieldByName
      ('driver').AsString;
    ACBrPosPrinter1.Ativar;

    if MemoryOutros.FieldByName('discont').AsFloat > 0 then
    begin
      TaxaDesconto := (MemoryOutros.FieldByName('discont').AsFloat);
    end;
    if MemoryOutros.FieldByName('entrega').AsFloat > 0 then
    begin
      TaxaEntrega := (MemoryOutros.FieldByName('entrega').AsFloat);
    end;
    if MemoryOutros.FieldByName('servico').AsFloat > 0 then
    begin
      TaxaEntrega := (TaxaEntrega + MemoryOutros.FieldByName('servico')
        .AsFloat);
    end;
    ...
    //
    /// / Corrigir a diferença acumulada
  ...
  Requisicao.Execute;
  try
    Serie := Requisicao.Retorno.ToInteger;
  except
  ...
  begin
    ...
    begin
      ...
      Ide.dhCont := date;
      Ide.xJust := 'Sem conexão com a internet!';
    end;

    Ide.tpAmb := Ambiente;

    ACBrNFe1.Configuracoes.WebServices.Ambiente := Ambiente;
    // Lembre-se de trocar esta variavel quando for para ambiente de producao
    Ide.cUF := UFtoCUF(UpperCase(MemoryConfiguracao.FieldByName('estado')
      .AsString));
    Ide.cMunFG := MemoryConfiguracao.FieldByName('codcidade').AsInteger;
    Ide.finNFe := fnNormal;
    Ide.tpImp := tiNFCe;
    ...
    Emit.CNPJCPF := MemoryConfiguracao.FieldByName('cnpj').AsString;
    Emit.IE := MemoryConfiguracao.FieldByName('ie').AsString;
    if length(Emit.IE) = 0 then
      Emit.IE := 'ISENTO';
    Emit.xNome := UpperCase(MemoryConfiguracao.FieldByName('razao').AsString);
    Emit.xFant := UpperCase(MemoryConfiguracao.FieldByName('nome').AsString);

    Emit.EnderEmit.fone := MemoryConfiguracao.FieldByName('fone').AsString;
    Emit.EnderEmit.CEP := StrToInt(MemoryConfiguracao.FieldByName('cep')
      .AsString);
    Emit.EnderEmit.xLgr := UpperCase(MemoryConfiguracao.FieldByName('rua')
      .AsString);
    Emit.EnderEmit.nro := MemoryConfiguracao.FieldByName('numero').AsString;
    // Emit.EnderEmit.xCpl := MemoryConfiguracao.FieldByName('complemento').AsString;
    Emit.EnderEmit.xBairro := UpperCase(MemoryConfiguracao.FieldByName('bairro')
      .AsString);
    Emit.EnderEmit.cMun := StrToInt(MemoryConfiguracao.FieldByName('codcidade')
      .AsString);
    Emit.EnderEmit.xMun := UpperCase(MemoryConfiguracao.FieldByName('cidade')
      .AsString);
    Emit.EnderEmit.UF := UpperCase(MemoryConfiguracao.FieldByName('estado')
      .AsString);
    ...
    Emit.CRT := StrToCRT(Ok, MemoryConfiguracao.FieldByName('tipo_empresa')
      .AsString);
    if MemoryOutros.RecordCount > 0 then
    begin
    ...
    begin
      ...
      begin
        ...
        mID_CST_PIS := StrToIntDef(Memory.FieldByName('cstpis').AsString, 0);
        mID_CST_COFINS := StrToIntDef(Memory.FieldByName('cstcofins')
          .AsString, 0);
        // Origem Mercadoria, falta colocar
        // mID_OrigemPro  := StrToIntDef(Memory.FieldByName('cstpis').AsString, 0);
        mID_ST_ICMS := StrToIntDef(Memory.FieldByName('csticms').AsString, 0);
        mID_CSOSN_ICMS := StrToIntDef(Memory.FieldByName('csosn').AsString, 0);
        mInd_Escala := 0;
        mCodANP := '';
        ...
        mQTDE := Memory.FieldByName('quanty').AsFloat;
        mValorUnit := Memory.FieldByName('value').AsFloat;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              if not ConverteOk then
                raise EDatabaseError.CreateFmt
                  ('Situação tributária no simples nacional "%s" desconhecida.',
            ...
            begin
              if ((TpcnCSTIcms(mID_ST_ICMS) in [cst60]) or
                (ICMS.CSOSN in [csosn500])) then
              begin
                // if (Ide.indFinal = cfNao) then
                // begin
                // ICMS.pST := Memory.FieldByName('icms').AsFloat + ICMS.pFCP;
          ...
          if not ConverteOk then
            raise EDatabaseError.CreateFmt
              ('Situação tributária do PIS "%s" desconhecida.', [mID_CST_PIS]);

          PIS.vBC := mVlrTotal;
          PIS.pPIS := Memory.FieldByName('cstpis').AsFloat;
          PIS.vPIS := Memory.FieldByName('pis').AsFloat;
          ...
          if not ConverteOk then
            raise EDatabaseError.CreateFmt
              ('Situação tributária do COFINS "%s" desconhecida.',
              [mID_CST_COFINS]);

          COFINS.vBC := mVlrTotal;
          COFINS.pCOFINS := Memory.FieldByName('cstcofins').AsFloat;
          COFINS.vCOFINS := Memory.FieldByName('cofins').AsFloat;
        ...
        begin
          ...
          // (QryVendaItem.FieldByName('IDPRODUTO').AsString);
        end;
        ...
        // if ((Memory.FieldByName('cfop').AsInteger = 5656) or
        // (Memory.FieldByName('cfop').AsInteger = 6656)) then
        // begin
        // Prod.comb.cProdANP := StrToIntDef(mCodANP, 0);
        // Prod.comb.UFcons := 'SC';
        // end;
        mCodANP := '';
        ValorTotal := ValorTotal + (Prod.vProd - Prod.vDesc) + Prod.vOutro;

        Prod.cEAN := Memory.FieldByName('bar').AsString;
    ...
    if TotalPagamento > ValorTotal then
    begin
      pag.vTroco := TotalPagamento - ValorTotal;
    end;
...
begin
  ...
  begin
    ...
    if pos(UpperCase('-nfe.xml'), UpperCase(NomeArq)) > 0 then
      NomeArq := StringReplace(NomeArq, '-nfe.xml', '-procNfe.xml',
        [rfIgnoreCase]);
...
var
  xTitulo, Chave, idLote, CNPJ, nSeqEvento, xUF, TipoAtor, VerAplic, AutXML,
    TipoAutoriz: string;
  Ok: Boolean;
begin
  {
  ...
  nSeqEvento := '1';
  if not(InputQuery(xTitulo, 'Sequencial do evento para o mesmo tipo de evento',
    nSeqEvento)) then
    exit;
  ...
  TipoAtor := '1';
  if not(InputQuery(xTitulo, 'Tipo de Ator do Evento (1, 2 ou 3)', TipoAtor))
  then
    exit;
...
begin
  Chave := '';
  if not(InputQuery('WebServices Eventos: Cancelamento', 'Chave da NF-e', Chave))
  then
    exit;
  Chave := Trim(OnlyNumber(Chave));
  idLote := '1';
  ...
  Protocolo := '';
  if not(InputQuery('WebServices Eventos: Cancelamento',
    'Protocolo de Autorização', Protocolo)) then
    exit;
...
begin
  ...
  // TpAmbToStr(ACBrNFe1.WebServices.Retorno.tpAmb));
...
begin
  Chave := '';
  if not(InputQuery('WebServices Eventos: Carta de Correção', 'Chave da NF-e',
    Chave)) then
    exit;
  Chave := Trim(OnlyNumber(Chave));
  idLote := '1';
...
begin
  ...
  MemoDados.Lines.Add('versao: ' +
    ACBrNFe1.WebServices.ConsultaCadastro.versao);
  MemoDados.Lines.Add('verAplic: ' +
    ACBrNFe1.WebServices.ConsultaCadastro.VerAplic);
  MemoDados.Lines.Add('cStat: ' +
    IntToStr(ACBrNFe1.WebServices.ConsultaCadastro.cStat));
  MemoDados.Lines.Add('xMotivo: ' +
    ACBrNFe1.WebServices.ConsultaCadastro.xMotivo);
  MemoDados.Lines.Add('DhCons: ' + DateTimeToStr
    (ACBrNFe1.WebServices.ConsultaCadastro.DhCons));
end;
...
begin
  ...
  // Define a chave da NFC-e a ser consultada
  ...
  begin
    ...
    begin
      // Define o caminho do arquivo onde o XML será salvo
  ...
  // Carrega a resposta no WebBrowser (se aplicável)
  LoadXML(ACBrNFe1.WebServices.Consulta.RetornoWS, WBResposta);
end;
...
begin
  ...
  MemoDados.Lines.Add('tpAmb: ' +
    TpAmbToStr(ACBrNFe1.WebServices.Recibo.tpAmb));
...
begin
  ...
  begin
    ...
    MemoDados.Lines.Add('tpAmb: ' +
      TpAmbToStr(ACBrNFe1.WebServices.Enviar.tpAmb));
    MemoDados.Lines.Add('verAplic: ' + ACBrNFe1.WebServices.Enviar.VerAplic);
    MemoDados.Lines.Add('cStat: ' +
      IntToStr(ACBrNFe1.WebServices.Enviar.cStat));
    ...
    begin
      if (ACBrIntegrador1.ComandoIntegrador.IntegradorResposta.Codigo <> '')
      then
      begin
        MemoResp.Lines.Add('[Integrador]');
        MemoResp.Lines.Add('Codigo=' + ACBrIntegrador1.ComandoIntegrador.
          IntegradorResposta.Codigo);
        MemoResp.Lines.Add('Valor=' + ACBrIntegrador1.ComandoIntegrador.
...
// else
// MemoDados.Lines.Add
// ('Atenção...: Este é o último lote de registros disponíveis para distribuição.');
//
// MemoDados.Lines.Add
...
// MemoDados.Lines.Add('');
//
// // Salva cada XML já descompactado
// for i := 0 to docZip.Count - 1 do
// begin
...
begin
  ...
  try
    ...
    // CC.Add('email_2@provedor.com.br');    // especifique um email valido
    ConfigurarEmail;
    ACBrNFe1.EnviarEmailEvento(Para, edtEmailAssunto.Text, mmEmailMsg.Lines, CC
      // Lista com emails que serao enviado copias - TStrings
      , nil // Lista de anexos - TStrings
      , nil // ReplyTo
      );
  finally
...
begin
  ...
  begin
    ...
    CarregarMaisXML := MessageDlg('Carregar mais Notas?', mtConfirmation,
...
begin
  ...
  // VerAplic);
  // MemoDados.Lines.Add('cStat: ' +
  // IntToStr(ACBrNFe1.WebServices.Inutilizacao.cStat));
  // MemoDados.Lines.Add('xMotivo: ' + ACBrNFe1.WebServices.Inutilizacao.xMotivo);
  // MemoDados.Lines.Add('cUF: ' +
  // IntToStr(ACBrNFe1.WebServices.Inutilizacao.cUF));
  // MemoDados.Lines.Add('Ano: ' +
  // IntToStr(ACBrNFe1.WebServices.Inutilizacao.Ano));
  // MemoDados.Lines.Add('CNPJ: ' + ACBrNFe1.WebServices.Inutilizacao.CNPJ);
  // MemoDados.Lines.Add('Modelo: ' +
  // IntToStr(ACBrNFe1.WebServices.Inutilizacao.modelo));
  // MemoDados.Lines.Add('Serie: ' +
  // IntToStr(ACBrNFe1.WebServices.Inutilizacao.Serie));
  // MemoDados.Lines.Add('NumeroInicial: ' +
  // IntToStr(ACBrNFe1.WebServices.Inutilizacao.NumeroInicial));
  // MemoDados.Lines.Add('NumeroInicial: ' +
  // IntToStr(ACBrNFe1.WebServices.Inutilizacao.NumeroFinal));
  // MemoDados.Lines.Add('dhRecbto: ' +
  // DateTimeToStr(ACBrNFe1.WebServices.Inutilizacao.dhRecbto));
  // MemoDados.Lines.Add('Protocolo: ' + ACBrNFe1.WebServices.Inutilizacao.
  // Protocolo);
end;
...
begin
  ...
  Titulo := 'Manifestação Destinatario - ' +
  ...
  begin
    lMsg := 'Id: ' + Id + #13 + 'tpAmb: ' + TpAmbToStr(tpAmb) + #13 +
      ...
      'xEvento: ' + xEvento + #13 + 'nSeqEvento: ' + IntToStr(nSeqEvento) + #13
      + 'CNPJDest: ' + CNPJDest + #13 + 'emailDest: ' + emailDest + #13 +
...
begin
  ...
  begin
    ...
    begin
      ...
      MemoDados.Lines.Add('Codigo=' + ACBrIntegrador1.ComandoIntegrador.
        IntegradorResposta.Codigo);
      MemoDados.Lines.Add('Valor=' + ACBrIntegrador1.ComandoIntegrador.
        IntegradorResposta.Valor);
  ...
  begin
    ...
    try
      ...
      begin
        ...
        MemoDados.Lines.Add('Erro: ' + ACBrNFe1.NotasFiscais.Items[0]
          .ErroValidacao);
        MemoDados.Lines.Add('Erro Completo: ' + ACBrNFe1.NotasFiscais.Items[0]
...
begin
  ...
  begin
    ...
    ACBrPosPrinter1.Device.ParamsString :=
      frmConfiguraSerial.Device.ParamsString;
  end;
end;
...
begin
  ...
  iContabilidade.BaseUrl := BaseUrl;
...
begin
  ...
  end;
  {
    ...
    iReq.BaseUrl := BaseUrl;
    iReq.URL := 'nfce/contabilidade';
    iReq.MemTable2 := Dados;
  ...
  // begin
  // ProcessaNFCe(MemoryConfiguracao.FieldByName('emailcontabilidade').AsString,
  // MemoryConfiguracao.FieldByName('cnpj').AsString,
  // MemoryConfiguracao.FieldByName('nome').AsString, Dados.FieldByName('data')
  // .AsString);
  // end;
...
begin
  ...
  if Windows.GetComputerName(Buffer, Size) then
    ...
    Result := 'Nome do computador não encontrado';
end;

procedure TfrmACBrNFe.GetDadosReq(Mem: TFDMemTable; URL: String);
var
  req: iRequisicao;
begin
...
begin
  ...
  try
    ...
    edtPathSchemas.Text := Ini.ReadString('Geral', 'PathSchemas',
      ...
      GetEnumName(TypeInfo(TpcnVersaoDF), Integer(cbVersaoDF.ItemIndex)));
    ...
    cbxSepararPorModelo.Checked := Ini.ReadBool('Arquivos',
...
begin
  ...
  begin
    ...
    if NaoEstaVazio(edtIntervalo.Text) then
      ...
      edtIntervalo.Text :=
        IntToStr(ACBrNFe1.Configuracoes.WebServices.IntervaloTentativas);
  ...
  begin
    ...
    ACBrNFe1.DANFE.MargemDireita := 5.5;
    ACBrNFe1.DANFE.MargemEsquerda := 5.5;
...
begin
  ...
  RESTRequest.Params.AddItem('hora', FormatDateTime('hh:nn:ss',
    ACBrNFe1.NotasFiscais.Items[0].NFe.Ide.dEmi),
    TRESTRequestParameterKind.pkGETorPOST); // Ajustado para HH:MM
  RESTRequest.Params.AddItem('valor',
    FloatToStrF(ACBrNFe1.NotasFiscais.Items[0].NFe.Total.ICMSTot.vNF, ffFixed,
    15, 2), TRESTRequestParameterKind.pkGETorPOST);
  RESTRequest.Params.AddItem('chaveNFCe', Chave,
    TRESTRequestParameterKind.pkGETorPOST);

  RESTRequest.Params.AddItem('name', GetComputerName,
    TRESTRequestParameterKind.pkGETorPOST);
  ...
  try
    ...
    begin
      ...
      try
        ...
        else if ResponseJSON.GetValue('error') <> nil then
        begin

          if (ResponseJSON.GetValue('error')
            .Value = 'Já existe uma nota fiscal com esta chave para o CNPJ informado.')
          then
          begin
            try
              iContabilidade.URL := 'nfce/nota/sinc/' + Chave;
            ...
            MemoResp.Lines.Add('Erro: ' + ResponseJSON.GetValue('error').Value)
        end
        else
          MemoResp.Lines.Add('Resposta inesperada.');
      finally
        ResponseJSON.Free;
      end;
    end
    else
      MemoResp.Lines.Add('Erro ao enviar nota fiscal: ' +
        RESTResponse.StatusText);

  except
    on E: Exception do
      MemoResp.Lines.Add('Erro: ' + E.Message);
  end;
end;

function TfrmACBrNFe.ExtrairChaveNFe(const ATexto: string): string;
var
  RegEx: TRegEx;
  Match: TMatch;
begin
  Result := '';
  RegEx := TRegEx.Create('\[chNFe:(.*?)\]');
...
begin
  ACBrUtil.FilesIO.WriteToTXT(PathWithDelim(ExtractFileDir(Application.ExeName))
    + 'temp.xml', ACBrUtil.XMLHTML.ConverteXMLtoUTF8(RetWS), False, False);
...
procedure TfrmACBrNFe.PostDadosReq(URL, BODY: String);
var
  req: iRequisicao;
begin
  req := iRequisicao.Create(nil);
...
begin
  ...
  ACBrPosPrinter1.PaginaDeCodigo := TACBrPosPaginaCodigo
...
procedure TfrmACBrNFe.ProcessaNFCe(EMAIL, CNPJ, NOME, Data: String);
var
  Caminho: String;
  CaminhoSalvo: String;
  req: iRequisicao;
  ...
  FilePath: string;
begin
  MemoResp.Lines.Add('Processando Email');
  req := iRequisicao.Create(nil);

  req.BaseUrl := BaseUrl;
  Caminho := edtPathNFe.Text + '\' + CNPJ + '\NFCe\' + Data + '\';
  ForceDirectories(edtPathNFe.Text + '\Email\');
  CaminhoSalvo := edtPathNFe.Text + '\Email\(' + CNPJ + ') ' + Data + '.zip';

  if not DirectoryExists(Caminho) then
  begin
    MemoResp.Lines.Add('Diretorio Não Existe!');
    req.URL := 'nfce/contabilidade/2/Diretorio não existe';
    req.Metodo := mPost;
    req.Execute;
    req.Free;
    exit
  ...
  try
    ...
    req.URL := 'nfce/contabilidade/notas/' + Data;
    req.Metodo := mGet;
    req.MemTable2 := Dados;
    req.Execute;
    ...
    on E: Exception do
    begin
      MemoResp.Lines.Add('Erro ao buscar as notas !');
      MemoResp.Lines.Add(E.Message);
      req.BaseUrl := BaseUrl;
      req.URL := 'nfce/contabilidade/2/Erro ao buscar as notas';
      req.Metodo := mPost;
      req.Execute;
      req.Free;
      exit
  ...
  try
    ZipFile.Open(CaminhoSalvo, zmWrite);
    while not Dados.Eof do
    begin
      FilePath := Caminho + Dados.FieldByName('chave').AsString + '-nfe.xml';
  ...
  // ZipFolder(Caminho, CaminhoSalvo);

  ACBrMail1.From := 'contabilidade@goopedir.com';
  ACBrMail1.FromName := 'GooPedir Contabil';
  ACBrMail1.Host := 'smtp.hostinger.com'; // troque pelo seu servidor smtp
  ACBrMail1.Username := 'contabilidade@goopedir.com';
  ACBrMail1.Password := 'MinhaContabilidade@2023';
  ...
  ACBrMail1.AddAddress('allan@goopedir.com', 'Allan');
  ACBrMail1.AddAddress('contabilidade@goopedir.com', 'Allan');
  ACBrMail1.AddAddress(EMAIL, 'Contabilidade');
  ACBrMail1.Subject := '(' + CNPJ + ') - ' + NOME + ' / ' + Data;
  ACBrMail1.AltBody.Text := 'Segue anexo XML das NFC-e emitidas no periodo ' +
    Data + #13 + #13 + '(' + CNPJ + ') - ' + NOME;

  ACBrMail1.AddAttachment(CaminhoSalvo, 'XML NFC-e', adAttachment);
  try
    ACBrMail1.Send;
  except
    on E: Exception do
  ...
  try
    MemoResp.Lines.Add(BaseUrl + '/nfce/contabilidade/1/Sucesso');
    req.BaseUrl := BaseUrl;
    req.TempoExpiracao := 60 * 1000;
    req.URL := '/nfce/contabilidade/1/Sucesso';
    req.Metodo := mPost;
    req.Execute;
    req.Free;
    ...
    on E: Exception do
    begin
      req.BaseUrl := 'nfce/contabilidade/2/' + E.Message;
      req.Metodo := mPost;
      req.Execute;
      req.Free;
...
function TfrmACBrNFe.SomenteNumeros(const AStr: string): string;
var
  i: Integer;
begin
  Result := '';
...
procedure TfrmACBrNFe.tEmissaoTimer(Sender: TObject);
var
  iReq: iRequisicao;
  Ambiente: String;
  Chave: String;
...
begin
  ...
  // iEmissao.Execute;
  //
  // if dadosEmissao.RecordCount > 0 then
  // begin
  //
  // while not dadosEmissao.Eof do
  // begin
  // if dadosEmissao.FieldByName('nfce_numero').IsNull then
  // begin
  // dadosEmissao.Edit;
  ...
  // ReqNFCE.BaseUrl := iCode.BaseUrl;
  // ReqNFCE.URL := '/impressao/pedido/nfce/' + dadosEmissao.FieldByName
  // ('codigo').AsString;
  ...
  try
    dadosCode.Close;
    iCode.MemTable2 := dadosCode;
    iCode.Execute;

    if dadosCode.RecordCount > 0 then
    begin
        ...
        begin
          ...
          begin
            ...
            begin
              infEvento.dhEvento := now;
              infEvento.tpEvento := teCancelamento;
              infEvento.detEvento.xJust := dadosCode.FieldByName('obs')
                .AsString;
            end;
            ...
            begin
              ...
              begin
                // Coloque aqui o código a ser executado se cStat for 573 ou 135
                case ACBrNFe1.NotasFiscais.Items[0].NFe.Ide.tpAmb of
                  taProducao:
                    begin
                      Ambiente := '1';
                ...
                iReq := iRequisicao.Create(nil);
                iReq.BaseUrl := BaseUrl;
                iReq.URL := 'nfce/emissao/' + dadosCode.FieldByName('id')
                  .AsString + '/' +
                  IntToStr(ACBrNFe1.NotasFiscais.Items[0].NFe.Ide.cNF) + '/' +
                  Chave + '/' + Protocolo + '/' + Ambiente;
                iReq.Metodo := mPost;
              ...
              begin
                ...
                Requisicao.URL := '/nfce/code/motivo';
                Requisicao.Metodo := mPost;
                Requisicao.BODY('{"chave":"' + dadosCode.FieldByName('chave')
                  .AsString + '","motivo":"' + EventoRetorno.retEvento.Items[0]
...
procedure TfrmACBrNFe.tEmissaoV2Timer(Sender: TObject);
var
  mLote: TFDMemTable;
  ErroEmissao: Boolean;
  Erro: String;
    ...
    begin
      try
        ...
        begin
          ...
          Erro := E.Message + ' - ' + ProdutoComErroFiscal;
          MemoResp.Lines.Add(Erro);
          PostDadosReq('v2/erro/nfce',
            '{"pedido":"' + mLote.FieldByName('codigo').AsString + '","erro":"'
            + E.Message + '"}');
        end;
      end;
      ...
      begin
        ...
        begin
          if mLote.FieldByName('nfce_numero').AsInteger = 0 then
          begin
            // ACBrNFe1.NotasFiscais.Imprimir;
          end;
        ...
        begin
          try
            ...
            on E: Exception do
            begin
              MemoResp.Lines.Add(E.Message + ' ' + mLote.FieldByName('codigo')
                .AsString);
              Erro := E.Message;
              if ExtrairNItemDoErro(Erro) > -1 then
              begin
          ...
          begin
            ...
            URL := 'nfce/emissao/' + mLote.FieldByName('codigo').AsString + '/'
              + IntToStr(0) + '/' + Chave + '/' + Protocolo + '/' + Ambiente;
            PostDadosReq(URL, '');
          ...
          begin
            // Ta sem log momentaneamente
            // EnviaGlitchtip('https://2321bb196f424d6aa9e80d51cc77273b@nginx-glitchtip.l1p88w.easypanel.host/6',CNPJ, dadosEmissao.FieldByName('codigo').AsString, Erro);
          end;
        end;
        if (ACBrNFe1.NotasFiscais.Items[0].NFe.Ide.tpEmis <> teContingencia)
        then
        ...
        URL := '/impressao/pedido/nfce/' + mLote.FieldByName('codigo').AsString;
        PostDadosReq(URL, '');
      end;
...
procedure TfrmACBrNFe.TrayIcon1Click(Sender: TObject);
begin
  case StatusForm of
    sVisivel:
      begin
...
procedure TfrmACBrNFe.ZipFolder(const SourceFolder, DestinationZip: string);
begin
  TZipFile.ZipDirectoryContents(DestinationZip, SourceFolder);
end;
